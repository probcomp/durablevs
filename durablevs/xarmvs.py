...
class XARMVS:
    def __init__(self):
        ...
        self.aruco_relative_poses = [
            ( [0.0, -1.1, 0.05], p.getQuaternionFromEuler([0.0, 0.0, 0.0])),
            ([1.1, -0.6, 0.0], p.getQuaternionFromEuler([0.0, 0.0, np.pi/4])),
            ([-1.1, -0.6, 0.0], p.getQuaternionFromEuler([0.0, 0.0, -np.pi/4]))
        ]
        self.home_jps = np.array([0.0, 0.5, -0.5,  0.0, 0.0, np.pi/2])
    ...
    def get_position(self):
        jps = np.zeros(p.getNumJoints(self.xarm)-1)
        for i in range(1,p.getNumJoints(self.xarm)):
            jps[i-1] =  p.getJointState(self.xarm, i)[0]
        return jps
    ...
    def go_to_position(self, jps):
        ...
        #     print(p.getJointInfo(xarm, i)[2])
            ...
            p.resetJointState(self.xarm, i, jps[i-1])
    ...
    def load_xarm(self):
        ...
        xarm = p.loadURDF("xarm/xarm6_robot.urdf", np.array([-0,-0,-9]), [0,0,0,1], useFixedBase=True, flags=flags, globalScaling=30.0)
        return xarm
